var MQ = {
  ...
  asArray: function asArray(obj, direction) {
    var bp = this.getBreakPoints(obj);
    var custom = this.getCustomQueries(obj);

    return this._translate(this._makeSteps(this._toSortedArray(bp)), direction)
      .concat(this._objToArr(custom));
  },

  asObject: function asObject(obj, value) {
    var direction = value && value === 'height' ? 'height' : 'width';

    return this._arrToObj(this.asArray(obj), direction);
  },
  ...
  _translate: function _translate(arr, value) {
    var direction = value && value === 'height' ? 'height' : 'width';

    if (arr === []) return arr;
    return arr.map(function (el, index) {
      return (index === 0)
        ? [el[0], 'screen and (max-' + direction + ': ' + el[1] + 'px)']
        : (index === arr.length - 1)
          ? [(el[0] || 'default'), 'screen and (min-' + direction + ': ' +
            (arr[index - 1][1] + 1) + 'px)']
          : [el[0], 'screen and (min-' + direction + ': ' + (arr[index-1][1] + 1) +
            'px) and (max-' + direction + ': ' + el[1] + 'px)'];
    });
  },
